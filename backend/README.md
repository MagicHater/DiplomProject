...
4. Проверяют TCP-логин через host-порт `5433` (временный `postgres:16` клиент).
5. Очищают конфликтующие `SPRING_*` переменные окружения в текущей сессии.
6. Запускают backend с явными `--spring.datasource.*` и `--spring.flyway.*` аргументами (`--no-daemon`).
   Для PowerShell-скрипта включён fail-fast: при ошибке docker/psql скрипт теперь падает сразу с понятным шагом.
   Если TCP auth-check не проходит, скрипт автоматически делает один повтор с `RESET_DB=1`.
