...
Проверьте, что в начале вывода есть строка `v3` и текущий commit (`[meta] ... commit: ...`).
Скрипт не использует жёсткий абсолютный путь: рабочая директория вычисляется относительно `backend/scripts`.
...
`backend/docker-compose.yml` для локальной разработки использует `POSTGRES_HOST_AUTH_METHOD=trust`.
Если меняли эту настройку, обязательно один раз запустите с `RESET_DB=1`, чтобы пересоздать volume с новыми правилами доступа.
...
3. Принудительно выставляют пароль пользователя `postgres` внутри контейнера в ожидаемое значение (`postgres`).
4. Проверяют TCP-логин внутри контейнера (`127.0.0.1:5432`) с тем же паролем.
5. Дополнительно делают host-path probe (`host.docker.internal:5433`) в режиме best-effort (warning-only).
6. Очищают конфликтующие `SPRING_*` переменные окружения в текущей сессии.
7. Запускают backend с явными `--spring.datasource.*` и `--spring.flyway.*` аргументами (`--no-daemon`).
   Для PowerShell-скрипта включён fail-fast: при ошибке docker/psql скрипт теперь падает сразу с понятным шагом.
